...
Extração: A ingestão ocorre via arquivos [CSV/JSON] ou conexão direta com banco de dados.

Transformação: O grupo aplica a limpeza de valores ausentes, a remoção de outliers e a normalização das escalas numéricas.

Carga: Os dados tratados são disponibilizados na pasta `/data/processed` para consumo dos modelos de Machine Learning.
